...
trait TestResult {
  ...
  def precision(testName: String = null): Double = {
    val (tp, fp) = Option(testName) match {
      ...
      case None =>
        metricsByTest.values.foldLeft((0, 0)) { case ((accTP, accFP), m) =>
          (accTP + m.truePositives, accFP + m.falsePositives)
        }
    }
    val denom = tp + fp
  ...
  def recall(testName: String = null): Double = {
    val (tp, fn) = Option(testName) match {
      ...
      case None =>
        metricsByTest.values.foldLeft((0, 0)) { case ((accTP, accFN), m) =>
          (accTP + m.truePositives, accFN + m.falseNegatives)
        }
    }
    val denom = tp + fn
    val value = denom match {
  ...
  def passRate(testName: String = null): Double = {
    val (passed, failed) = Option(testName) match {
      ...
      case None =>
        metricsByTest.values.foldLeft((0, 0)) {
          case ((accPassed, accFailed), m) =>
            (accPassed + m.passedTests, accFailed + m.failedTests)
        }
    }
  ...
  def vulnerabilitiesFound(testName: String = null): Int = Option(
    testName
  ) match {
  ...
  def report(testName: String): Unit = {
    ...
    println(
      s"failed = ${m.failedTests}, passed = ${m.passedTests} of = ${m.passedTests + m.failedTests} tests."
    )
    println(s"Pass Rate = ${passRate(testName)}%")
    println(
      s"Expecting ${vulnerabilities(testName)} of ${vulnerabilitiesFound(testName)} warnings."
    )
    println(
      s"TP = ${m.truePositives} FP = ${m.falsePositives} FN = ${m.falseNegatives} TN = ${m.trueNegatives}"
    )
    println(
      s"Precision = ${precision(testName)}% Recall = ${recall(testName)}% F-score = ${f1Score(testName)}%"
    )
  ...
  def reportSummary(reportName: String): Unit = {
    ...
    println(
      s"- **$reportName** - failed: ${metricsByTest.values.count(_.failedTests > 0)}, passed: ${metricsByTest.values.count(_.passedTests > 0)} of ${metricsByTest.values.size} tests - (${passRate()}%)"
    )
